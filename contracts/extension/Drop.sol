...
abstract contract Drop is IDrop {
    ...
    function setClaimConditions(
        ...
        for (uint256 i = 0; i < _conditions.length; i++) {
            ...
            uint256 supplyClaimedAlready = claimCondition
        ...
        if (_resetClaimEligibility) {
            ...
            if (existingPhaseCount > _conditions.length) {
                for (
                    uint256 i = _conditions.length;
                    i < existingPhaseCount;
                    i++
                ) {
    ...
    function verifyClaim(
        ...
        ClaimCondition memory currentClaimPhase = claimCondition.conditions[
            _conditionId
        ];
        ...
        if (
            _quantity == 0 || (_quantity + supplyClaimedByWallet > claimLimit)
        ) {
            revert("!Qty");
        }
        if (
    ...
    function getActiveClaimConditionId() public view returns (uint256) {
        for (
            ...
            if (
                block.timestamp >=
                claimCondition.conditions[i - 1].startTimestamp
            ) {
                return i - 1;
            }
        }
    ...
    {
        supplyClaimedByWallet = claimCondition.supplyClaimedByWallet[
            _conditionId
        ][_claimer];
    }
